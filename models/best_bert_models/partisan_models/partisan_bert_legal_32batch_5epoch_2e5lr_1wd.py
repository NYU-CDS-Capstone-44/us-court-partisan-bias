...
version = '32batch_5epoch_2e5lr_1wd'
...
# Load in data
val = pd.read_csv('/vast/amh9750/sc-val.csv')
val = val.drop(val.columns[0], axis=1)
decision_directions = np.array(val['scdb_decision_direction'])
decision_directions = (decision_directions == 1.0).astype(int)
val['labels'] = decision_directions
test = pd.read_csv('/vast/amh9750/sc-test.csv')
test = test.drop(test.columns[0], axis=1)
decision_directions = np.array(test['scdb_decision_direction'])
decision_directions = (decision_directions == 1.0).astype(int)
test['labels'] = decision_directions

train = pd.read_csv('/scratch/amh9750/us-court-partisan-bias/summarized-opinions-data-sc.csv')
...
decision_directions = np.array(train['scdb_decision_direction'])
decision_directions = (decision_directions == 1.0).astype(int)
train['labels'] = decision_directions
...
# Load in model
model_str = "nlpaueb/legal-bert-base-uncased"
...
model_name = f"partisan-legal-bert-base-uncased-supreme-court-{version}"
...
finetune_model_name = f'annabellehuether/partisan-legal-bert-base-uncased-supreme-court-{version}'
...
file_name = f'partisan_legal_bert_metrics_{version}.txt'
...
test_results.to_csv(f'partisan_legal_bert_test_results_{version}.csv')
